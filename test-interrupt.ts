...
async function main(): Promise<void> {
	...
	try {
		...
		if (process.platform === "win32") {
			// Node cannot deliver a catchable SIGINT to a Windows child process.
			// Timeouts therefore hard-kill the kernel immediately and preserve only
			// output, not namespace state. Verify that behavior is explicit rather
			// than surfacing as a rejected "kernel exited" tool call.
			console.log("\n[windows] timeout hard-kills and cleanly respawns");
			const timedOutPid = k.getInfo()?.pid;
			const timedOut = await k.execute(
				[{ code: "print('before hard kill', flush=True)\nimport time\ntime.sleep(30)" }],
				{ timeoutMs: 500, interruptGraceMs: 5_000 },
			);
			check(timedOut.timedOut, "result.timedOut === true");
			check(timedOut.killed === true, "result.killed === true");
			check(
				(timedOut.cells[0]?.stdout ?? "").includes("before hard kill"),
				"partial output survives the hard kill",
			);
			check(!k.isAlive(), "kernel is dead after timeout");

			const fresh = await k.execute([{ code: "'respawned'" }], { timeoutMs: 5_000 });
			check(fresh.cells[0]?.value === "'respawned'", "next call respawns cleanly");
			check(k.getInfo()?.pid !== timedOutPid, "respawn uses a new pid");
		} else {
	...
	const k2 = new PythonKernel({ pythonPath, cwd: process.cwd() });
	await k2.start();
